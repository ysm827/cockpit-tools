...
import { CodebuddyAccountsContent, type CodebuddyAccountsContentTab } from '../../pages/CodebuddyAccountsContent';
import './style.css';

type CodebuddyRemoteHostApi = {
...
function CodebuddyRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: CodebuddyAccountsContentTab;
  onTabChange: (tab: CodebuddyAccountsContentTab) => void;
}) {
...
function CodebuddyRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<CodebuddyAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <CodebuddyRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <CodebuddyAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: CodebuddyRemoteHostApi) {
  unmount(container);
  ...
  root.render(<CodebuddyRemoteApp tabsContainer={tabsContainer} />);
