...
import { CodebuddyCnAccountsContent, type CodebuddyCnAccountsContentTab } from '../../pages/CodebuddyCnAccountsContent';
import './style.css';

type CodebuddyCnRemoteHostApi = {
...
function CodebuddyCnRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: CodebuddyCnAccountsContentTab;
  onTabChange: (tab: CodebuddyCnAccountsContentTab) => void;
}) {
...
function CodebuddyCnRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<CodebuddyCnAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <CodebuddyCnRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <CodebuddyCnAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: CodebuddyCnRemoteHostApi) {
  unmount(container);
  ...
  root.render(<CodebuddyCnRemoteApp tabsContainer={tabsContainer} />);
