...
import { TraeAccountsContent, type TraeAccountsContentTab } from '../../pages/TraeAccountsContent';
import './style.css';

type TraeRemoteHostApi = {
  platformId: 'trae';
...
function TraeRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: TraeAccountsContentTab;
  onTabChange: (tab: TraeAccountsContentTab) => void;
}) {
...
function TraeRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<TraeAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <TraeRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <TraeAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: TraeRemoteHostApi) {
  unmount(container);
  ...
  root.render(<TraeRemoteApp tabsContainer={tabsContainer} />);
