...
import {
  GitHubCopilotAccountsContent,
  type GitHubCopilotAccountsContentTab,
} from '../../pages/GitHubCopilotAccountsContent';
import './style.css';

type GitHubCopilotRemoteHostApi = {
...
function GitHubCopilotRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: GitHubCopilotAccountsContentTab;
  onTabChange: (tab: GitHubCopilotAccountsContentTab) => void;
}) {
...
function GitHubCopilotRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<GitHubCopilotAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <GitHubCopilotRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <GitHubCopilotAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: GitHubCopilotRemoteHostApi) {
  unmount(container);
  ...
  root.render(<GitHubCopilotRemoteApp tabsContainer={tabsContainer} />);
