...
import { QoderAccountsContent, type QoderAccountsContentTab } from '../../pages/QoderAccountsContent';
import './style.css';

type QoderRemoteHostApi = {
  platformId: 'qoder';
...
function QoderRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: QoderAccountsContentTab;
  onTabChange: (tab: QoderAccountsContentTab) => void;
}) {
...
function QoderRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<QoderAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <QoderRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <QoderAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: QoderRemoteHostApi) {
  unmount(container);
  ...
  root.render(<QoderRemoteApp tabsContainer={tabsContainer} />);
