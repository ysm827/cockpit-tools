...
import { WorkbuddyAccountsContent, type WorkbuddyAccountsContentTab } from '../../pages/WorkbuddyAccountsContent';
import './style.css';

type WorkbuddyRemoteHostApi = {
...
function WorkbuddyRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: WorkbuddyAccountsContentTab;
  onTabChange: (tab: WorkbuddyAccountsContentTab) => void;
}) {
...
function WorkbuddyRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<WorkbuddyAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <WorkbuddyRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <WorkbuddyAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: WorkbuddyRemoteHostApi) {
  unmount(container);
  ...
  root.render(<WorkbuddyRemoteApp tabsContainer={tabsContainer} />);
