...
import { GeminiAccountsContent, type GeminiAccountsContentTab } from '../../pages/GeminiAccountsContent';
import './style.css';

type GeminiRemoteHostApi = {
  platformId: 'gemini';
...
function GeminiRemoteTabs({
  activeTab,
  onTabChange,
}: {
  activeTab: GeminiAccountsContentTab;
  onTabChange: (tab: GeminiAccountsContentTab) => void;
}) {
    ...
    () => [
      {
        ...
        icon: <GeminiIcon className="tab-icon" />,
      },
      {
...
function GeminiRemoteApp({
  tabsContainer,
}: {
  tabsContainer: HTMLElement | null;
}) {
  const [activeTab, setActiveTab] = useState<GeminiAccountsContentTab>('overview');

  return (
    <React.StrictMode>
      {tabsContainer
        ? createPortal(
            <GeminiRemoteTabs activeTab={activeTab} onTabChange={setActiveTab} />,
            tabsContainer,
          )
        : null}
      <GeminiAccountsContent activeTab={activeTab} />
    </React.StrictMode>
  );
}

export async function mount(container: HTMLElement, hostApi: GeminiRemoteHostApi) {
  unmount(container);
  ...
  root.render(<GeminiRemoteApp tabsContainer={tabsContainer} />);
