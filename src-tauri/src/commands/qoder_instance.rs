...
#[tauri::command]
pub async fn qoder_get_instance_defaults() -> Result<InstanceDefaults, String> {
    qoder_call_async("instance.getDefaults", json!({})).await
}

#[tauri::command]
pub async fn qoder_list_instances() -> Result<Vec<InstanceProfileView>, String> {
    qoder_call_async("instance.list", json!({})).await
}
...
pub async fn qoder_delete_instance(instance_id: String) -> Result<(), String> {
    qoder_call_async("instance.delete", json!({ "instanceId": instance_id })).await
}

#[tauri::command]
pub async fn qoder_start_instance(instance_id: String) -> Result<InstanceProfileView, String> {
    qoder_call_async("instance.start", json!({ "instanceId": instance_id })).await
}

#[tauri::command]
pub async fn qoder_stop_instance(instance_id: String) -> Result<InstanceProfileView, String> {
    qoder_call_async("instance.stop", json!({ "instanceId": instance_id })).await
}

#[tauri::command]
pub async fn qoder_open_instance_window(instance_id: String) -> Result<(), String> {
    qoder_call("instance.openWindow", json!({ "instanceId": instance_id }))
}

#[tauri::command]
pub async fn qoder_close_all_instances() -> Result<(), String> {
    qoder_call_async("instance.closeAll", json!({})).await
}
